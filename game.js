class SoccerEmpireGame {
    constructor() {
        ...
        this.globalLeaderboardUrl = 'https://api.soccerempire.com/leaderboard'; 
        ...
        setInterval(() => this.syncGlobalLeaderboard(), 60000); 
    }
    ...
    initializeElements() {
        ...
        this.rebirthButton = document.createElement('button');
        this.rebirthButton.id = 'rebirth-button';
        this.rebirthButton.className = 'disabled';
        this.rebirthButton.textContent = 'Transfer to New Club';

        this.rebirthInfo = document.createElement('div');
        this.rebirthInfo.id = 'rebirth-info';

        const mainContent = document.getElementById('main-game-content');
        mainContent.insertBefore(this.rebirthInfo, document.getElementById('upgrades-section'));
        mainContent.insertBefore(this.rebirthButton, this.rebirthInfo);
    ...
    setupEventListeners() {
        ...
        this.rebirthButton.addEventListener('click', () => this.attemptRebirth());
    }
...
location.reload();
    ...
    updateRebirthDisplay() {
        const nextRebirthCost = this.rebirthCost * Math.pow(2, this.rebirthPoints);
        this.rebirthInfo.innerHTML = `
            <div>Current Club Prestige: ${this.rebirthPoints}</div>
            <div>Global Multiplier: x${this.rebirthMultiplier.toFixed(1)}</div>
            <div>Points needed for next transfer: ${this.formatNumber(nextRebirthCost)}</div>
        `;
        
        this.rebirthButton.className = this.points >= nextRebirthCost ? '' : 'disabled';
    }

    attemptRebirth() {
        const nextRebirthCost = this.rebirthCost * Math.pow(2, this.rebirthPoints);
        if (this.points >= nextRebirthCost) {
            this.rebirthPoints++;
            this.rebirthMultiplier = 1 + (this.rebirthPoints * 0.5); 
            this.resetGameWithMultiplier();
            this.updateRebirthDisplay();
        }
    }

    resetGameWithMultiplier() {
        this.points = 0;
        this.pointRate = 0;
        this.clickMultiplier = 1 * this.rebirthMultiplier;
        this.upgrades.forEach(upgrade => {
            upgrade.level = 0;
        });
        this.updateDisplay();
    ...
    detectRegion() {
        return Promise.resolve({ country: 'Global', region: 'Global', city: 'Global' });
    }
    ...
    loadGame() {
        ...
        if (savedGame) {
            ...
            this.rebirthPoints = parsedGame.rebirthPoints || 0;
            this.rebirthMultiplier = parsedGame.rebirthMultiplier || 1;
            this.updateDisplay();
            this.updateRebirthDisplay();
            this.renderUpgrades();
        }
